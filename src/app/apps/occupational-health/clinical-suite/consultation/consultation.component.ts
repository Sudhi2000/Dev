...
export class ConsultationComponent implements OnInit {
  ...
  columns: ListColumn[] = [
    ...
    { name: 'Employee Division', property: 'division', visible: true, isModelProperty: true },
  ...
  get_clinical_history(pageEvent: PageEvent) {
    ...
    this.clinicalService.get_consultation_register(this.doctor, startIndex, pageEvent.pageSize).subscribe({
      next: (result: any) => {
  ...
  get_unit_specific_clinical_history(pageEvent: PageEvent) {
    ...
    const endIndex = startIndex + pageEvent.pageSize;
    localStorage.setItem('pageIndex', JSON.stringify(pageEvent.pageIndex));
    this.clinicalService.get_unit_specific_consultation_register(this.doctor, startIndex, pageEvent.pageSize, this.userDivision).subscribe({
      next: (result: any) => {
        const data = result.data.map((elem: any) => elem.attributes);
        this.clinicalRegister.splice(startIndex, endIndex, ...data);
        this.totalItems = result.meta.pagination.total;
  ...
  view(data: any) {
    ...
    this.clinicalService.get_clinical_reference(data.patient_id).subscribe({
      next: (result: any) => {
        this.router.navigate(["/apps/occupational-health/clinical-suite/consultation/view/" + result.data[0].id])
      },
      error: (err: any) => {
        this.router.navigate(["/error/internal"])
      },
  ...
  action(data: any) {
    ...
    this.clinicalService.get_clinical_suite_refe(data.patient_id).subscribe({
      next: (result: any) => {
        this.router.navigate(["/apps/occupational-health/clinical-suite/doctor-consultation/" + result.data[0].id])
      },
      error: (err: any) => { },
      complete: () => { }
    })
