...
export class QueueComponent implements OnInit {
        ...
        if (status === false) {
          ...
          if (this.unitSpecific) {
            ...
            if (this.corporateUser) {
              ...
              this.get_external_audit_queue({ pageIndex: prevPageIndex, pageSize: this.pageSize, length: 0 })
              ...
              const prevPageIndex = Number(localStorage.getItem('pageIndex')) || 0;
              this.get_external_audit_unit_specific_queue({ pageIndex: prevPageIndex, pageSize: this.pageSize, length: 0 })
            }
          } else {
            const prevPageIndex = Number(localStorage.getItem('pageIndex')) || 0;
            this.get_external_audit_queue({ pageIndex: prevPageIndex, pageSize: this.pageSize, length: 0 })
  ...
  get_external_audit_queue(pageEvent: PageEvent) {
    this.isLoading = true;
    const startIndex = pageEvent.pageIndex * pageEvent.pageSize;
    const endIndex = startIndex + pageEvent.pageSize;
    localStorage.setItem('pageIndex', JSON.stringify(pageEvent.pageIndex));
    this.externalAuditService.get_audit_queue(this.userID, startIndex, pageEvent.pageSize).subscribe({
      next: (result: any) => {
  ...
  get_external_audit_unit_specific_queue(pageEvent: PageEvent) {
    this.isLoading = true;
    const startIndex = pageEvent.pageIndex * pageEvent.pageSize;
    const endIndex = startIndex + pageEvent.pageSize;
    localStorage.setItem('pageIndex', JSON.stringify(pageEvent.pageIndex));
    this.externalAuditService.get_audit_unit_specific_queue(this.userID, startIndex, pageEvent.pageSize, this.userDivision).subscribe({
      next: (result: any) => {
        const data = result.data.map((elem: any) => elem.attributes);
        this.externalAuditRegister.splice(startIndex, endIndex, ...data);
        this.totalItems = result.meta.pagination.total;
  ...
  generate() {
    ...
    if (this.unitSpecific && !this.corporateUser) {
      this.externalAuditService.get_external_audit_unit_specific_queue_search(this.serachReference, this.userDivision).subscribe({
        next: (result: any) => {
          const data = result.data.map((elem: any) => elem.attributes);
      ...
      this.externalAuditService.get_external_audit_queue_search(this.serachReference).subscribe({
        next: (result: any) => {
          const data = result.data.map((elem: any) => elem.attributes);
          this.externalAuditRegister = data
