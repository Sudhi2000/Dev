...
export class QueueComponent implements OnInit {
  ...
  columns: ListColumn[] = [
    ...
    { name: 'approval_date', property: 'approval_date', visible: true, isModelProperty: true },
    ...
    this.authService.me().subscribe({
      next: (result: any) => {
        ...
        if (status === false) {
          ...
          //     this.get_internal_audit_queue({ pageIndex: 0, pageSize: this.pageSize, length: 0 })
          ...
          //     this.get_internal_audit_unit_specific_queue({ pageIndex: 0, pageSize: this.pageSize, length: 0 })
          //   }
          // }
          // else {
          const prevPageIndex = Number(localStorage.getItem('pageIndex')) || 0;
          this.get_internal_audit_queue({ pageIndex: prevPageIndex, pageSize: this.pageSize, length: 0 })
          // }
        }
      },
      error: (err: any) => {
  ...
  get_internal_audit_queue(pageEvent: PageEvent) {
    this.isLoading = true;
    const startIndex = pageEvent.pageIndex * pageEvent.pageSize;
    const endIndex = startIndex + pageEvent.pageSize;
    localStorage.setItem('pageIndex', JSON.stringify(pageEvent.pageIndex));
    this.internalAuditService.get_audit_queue(this.userID, startIndex, pageEvent.pageSize).subscribe({
      next: (result: any) => {
  ...
  get_internal_audit_unit_specific_queue(pageEvent: PageEvent) {
    this.isLoading = true;
    const startIndex = pageEvent.pageIndex * pageEvent.pageSize;
    const endIndex = startIndex + pageEvent.pageSize;
    localStorage.setItem('pageIndex', JSON.stringify(pageEvent.pageIndex));
    this.internalAuditService.get_audit_unit_specific_queue(this.userID, startIndex, pageEvent.pageSize, this.userDivision).subscribe({
      next: (result: any) => {
        const data = result.data.map((elem: any) => elem.attributes);
        this.internalAuditRegister.splice(startIndex, endIndex, ...data);
        this.totalItems = result.meta.pagination.total;
  ...
  generate() {
    ...
    if (this.unitSpecific && !this.corporateUser) {
      this.internalAuditService.get_internal_audit_unit_specific_queue_search(this.serachReference, this.userID, this.userDivision).subscribe({
        next: (result: any) => {
          const data = result.data.map((elem: any) => elem.attributes);
      ...
      this.internalAuditService.get_internal_audit_queue_search(this.serachReference, this.userID).subscribe({
        next: (result: any) => {
          const data = result.data.map((elem: any) => elem.attributes);
          this.internalAuditRegister = data
        },
