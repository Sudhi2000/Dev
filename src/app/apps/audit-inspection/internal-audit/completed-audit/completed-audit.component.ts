...
import { L, N } from '@angular/cdk/keycodes';
...
export class CompletedAuditComponent implements OnInit {
      ...
      next: (result: any) => {
        ...
        if (result.data.length > 0) {
          ...
          if ((result.data[0].attributes.status === "Completed")
            ...
            if (
              result.data[0].attributes.labor_audit_status === 'Completed' &&
              result.data[0].attributes.health_audit_status === 'Completed' &&
              result.data[0].attributes.environment_audit_status ===
              'Completed' &&
              result.data[0].attributes.management_audit_status === 'Completed' &&
              result.data[0].attributes.security_audit_status === 'Completed'
            ) {
  ...
  progress(elem: any) {
    ...
    //   return elem.attributes.sub_category === subCategory;
    // });
    // const total = Number(data.length) * 3;
    // const earnedMark = data.reduce(
    //   (accumulator, current) => accumulator + Number(current.attributes.mark),
    //   0
    // );
    // const percentage = Number(
    //   (Number(earnedMark) / Number(total)) * 100
    // ).toFixed(0);
  ...
  auditMark(data: any, mark: any) {
    ...
    this.achievable_score = Number(data.attributes.achievable_score);
    ...
    if (mark.target.value === 'Not Applicable') {
      ...
      if (mark.target.value === 'Yes') {
        ...
        const remarks = '';
        this.internalAuditService
          .update_audit_mark(data.id, rating, score, mark.target.value, remarks)
          .subscribe({
            next: (result: any) => { },
            error: (err: any) => { },
            complete: () => {
          ...
          if (remarksresult) {
            ...
            this.internalAuditService
              ...
              .subscribe({
                ...
                error: (err: any) => { },
                complete: () => {
                  //check in action plan exist if exist update / create
  ...
  //audit status updation in pending_audit
