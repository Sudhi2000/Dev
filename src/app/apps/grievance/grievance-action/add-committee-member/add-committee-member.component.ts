...
@Component({
  ...
  styleUrls: ['./add-committee-member.component.scss']
})
export class AddCommitteeMemberComponent implements OnInit {
  ...
  constructor(@Inject(MAT_DIALOG_DATA) public defaults: any,
    ...
    public dialogRef: MatDialogRef<AddCommitteeMemberComponent>) { }
  ...
  submit() {
    ...
    this.Form.controls['department'].enable()
    if (this.employeeDetails.length > 0) {
      this.grievanceService.create_committee_member(this.Form.value).subscribe({
        next: (result: any) => { },
        error: (err: any) => {
          this.router.navigate(["/error/internal"])
        },
