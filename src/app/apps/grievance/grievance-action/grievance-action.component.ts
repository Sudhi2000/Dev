...
import { AddCommitteeMemberComponent } from './add-committee-member/add-committee-member.component';
...
import { AddLegalAdvisorComponent } from './add-legal-advisor/add-legal-advisor.component';
...
@Component({
  selector: 'app-grievance-action',
  templateUrl: './grievance-action.component.html',
  styleUrls: ['./grievance-action.component.scss'],
  providers: [
    { provide: DateAdapter, useClass: MomentDateAdapter, deps: [MAT_DATE_LOCALE] },
    { provide: MAT_DATE_FORMATS, useValue: MY_FORMATS }
  ]
})
export class GrievanceActionComponent implements OnInit {
  evidences: any = []
  ...
  committeeFormData = new FormData()
  evidenceData: any
  ...
  ngOnInit(): void {
    ...
    this.solutionForm = this.formBuilder.group({
      ...
      grievant_statement_doc: [''],
      grievant_statement: ['', [Validators.required]],
      commitee_statement: [''],
      ...
      grievance_stmt_id: [''],
      grievance_stmt_name: [''],
      grievance_stmt_format: [''],
      grievance_committee_stmt_id: [''],
      grievance_committee_stmt_name: [''],
      grievance_committee_stmt_format: [''],
      committee_stmt_id: [''],
      stmt_id: [''],
      committee_statement: [null, [Validators.required]],
    })
    ...
    this.grievanceService.get_grievance_details(reference).subscribe({
      next: (result: any) => {
        ...
        if (!matchFound || matchFound !== true) {
          ...
          this.solutionForm.controls['previously_raised'].setValue(result.data[0].attributes.previously_raised)
          this.solutionForm.controls['union_representative'].setValue(result.data[0].attributes.union_representative?.data?.id)
          this.solutionForm.controls['grievant_statement'].setValue(result.data[0].attributes.grievant_statement)
          ...
          this.solutionForm.controls['appeal'].setValue(result.data[0].attributes.appeal)
          this.solutionForm.controls['committee_statement'].setValue(result.data[0].attributes.committee_statement)
          ...
          this.memberList = result.data[0].attributes?.committee_members?.data
          this.nomineeList = result.data[0].attributes?.nominee?.data
          this.respondentList = result.data[0].attributes?.respondent?.data
          this.advisorList = result.data[0].attributes?.legal_advisors?.data
          ...
          const grievanceStatements = result.data[0].attributes.grievance_statements?.data;
          if (grievanceStatements) {
            grievanceStatements.forEach((statement: any) => {
              const statementName = statement.attributes.statement_name;
              const statementFormat = statement.attributes.format;
              const statementId = statement.attributes.statement_id;
              const stateId = statement.id;
              const isInvestigationCommittee = statement.attributes.investigation_committee;

              if (isInvestigationCommittee === false) {
                this.solutionForm.controls['grievance_stmt_name'].setValue(statementName);
                this.solutionForm.controls['grievance_stmt_format'].setValue(statementFormat);
                this.solutionForm.controls['grievance_stmt_id'].setValue(statementId);
                this.solutionForm.controls['stmt_id'].setValue(stateId);
              } else if (isInvestigationCommittee === true) {
                this.solutionForm.controls['grievance_committee_stmt_name'].setValue(statementName);
                this.solutionForm.controls['grievance_committee_stmt_format'].setValue(statementFormat);
                this.solutionForm.controls['grievance_committee_stmt_id'].setValue(statementId);
                this.solutionForm.controls['committee_stmt_id'].setValue(stateId);
              }
            });
          }
  ...
  addRespondent() {
    this.dialog.open(AddRespondentComponent, { data: this.Form.value }).afterClosed().subscribe(data => {
      const reference = this.route.snapshot.paramMap.get('id');
      this.grievanceService.get_grievance_details(reference).subscribe({
        next: (result: any) => {
          this.respondentList = result.data[0].attributes?.respondent?.data
  ...
  deleteRespondent(data: any) {
    this.grievanceService.delete_respondent(data.id).subscribe({
      next: (result: any) => { },
      error: (err: any) => {
        this.router.navigate(["/error/internal"])
      },
  ...
  addMember() {
    this.dialog.open(AddCommitteeMemberComponent, { data: this.Form.value.id }).afterClosed().subscribe(data => {
      const reference = this.route.snapshot.paramMap.get('id');
      this.grievanceService.get_grievance_details(reference).subscribe({
        next: (result: any) => {
          this.memberList = result.data[0].attributes?.committee_members?.data
  ...
  deleteMember(data: any) {
    this.grievanceService.delete_committee_member(data.id).subscribe({
      next: (result: any) => { },
      error: (err: any) => {
        this.router.navigate(["/error/internal"])
      },
  ...
  addLegalAdvisor() {
    this.dialog.open(AddLegalAdvisorComponent, { data: this.Form.value.id }).afterClosed().subscribe(data => {
      const reference = this.route.snapshot.paramMap.get('id');
      this.grievanceService.get_grievance_details(reference).subscribe({
        next: (result: any) => {
          this.advisorList = result.data[0].attributes?.legal_advisors?.data
  ...
  deleteLegalAdvisor(data: any) {
    this.grievanceService.delete_legal_advisor(data.id).subscribe({
      next: (result: any) => { },
      error: (err: any) => {
        this.router.navigate(["/error/internal"])
      },
  ...
  addNominee() {
    this.dialog.open(AddNomineeComponent, { data: this.Form.value.id }).afterClosed().subscribe(data => {
      const reference = this.route.snapshot.paramMap.get('id');
      this.grievanceService.get_grievance_details(reference).subscribe({
        next: (result: any) => {
          this.nomineeList = result.data[0].attributes?.nominee?.data
  ...
  deleteNominee(data: any) {
    this.grievanceService.delete_nominee(data.id).subscribe({
      next: (result: any) => { },
      error: (err: any) => {
        this.router.navigate(["/error/internal"])
      },
  ...
  new_employee() {
    ...
    this.dialog.open(AddEmployeeComponent).afterClosed().subscribe((data: any) => {

      this.grievanceService.create_employee(data).subscribe({
        next: (result: any) => {
          this.generalService.get_employees().subscribe({
            next: (result: any) => {

              this.tradeUnion = result.data
              this.solutionForm.controls['union_representative'].setValue(result.data[0].id)
            },
            error: (err: any) => {
              this.router.navigate(["/error/internal"])
            },
            complete: () => {
              const statusText = "Employee created successfully"
  ...
  onSelectStatement(event: any) {
    const fileLength = this.statementfiles.length;
    const addedLength = event.addedFiles.length;

    if (fileLength === 0 && addedLength === 1) { // Allow only one document to be selected
      const selectedFile = event.addedFiles[0];
      const size = selectedFile.size / (1024 * 1024); // Convert bytes to megabytes
      ...
      if (size > 5) {
        ...
        if (isSuccess) {
          this.statementfiles = [selectedFile]; // Replace the array with the single selected file
          this.solutionForm.controls['grievant_statement_doc'].setErrors(null);
          this.Upload_statement()
        } else {
      ...
      const statusText = "You can upload only one document";
      this._snackBar.open(statusText, 'Close Warning', {
        horizontalPosition: this.horizontalPosition,
        verticalPosition: this.verticalPosition,
      });
    } else if (addedLength > 1) {
      const statusText = "You can select only one document";
      this._snackBar.open(statusText, 'Close Warning', {
        horizontalPosition: this.horizontalPosition,
        verticalPosition: this.verticalPosition,
      });
  ...
  onRemoveStatement() {
    ...
    const stateID = this.solutionForm.value.stmt_id
    this.grievanceService.delete_grievance_statement(stateID).subscribe({
      next: (result: any) => {
      ...
      complete: () => {
        Swal.close()
        const statusText = "Grievance Statement deleted"
        this._snackBar.open(statusText, 'Close Warning', {
          horizontalPosition: this.horizontalPosition,
          verticalPosition: this.verticalPosition,
        });
        this.solutionForm.controls['grievance_stmt_id'].reset();
      }
    })
  }

  removeCommitteeStatement() {
    this.showProgressPopup()
    const stateID = this.solutionForm.value.committee_stmt_id
    this.grievanceService.delete_grievance_statement(stateID).subscribe({
      next: (result: any) => {
      ...
      complete: () => {
        Swal.close()
        const statusText = "Investigation Committee Statement deleted"
        this._snackBar.open(statusText, 'Close Warning', {
          horizontalPosition: this.horizontalPosition,
          verticalPosition: this.verticalPosition,
        });
        this.solutionForm.controls['grievance_committee_stmt_id'].reset();
      }
    })
  }
  ...
  selectCommitteeStatement(event: any) {
    const fileLength = this.committeefiles.length;
    const addedLength = event.addedFiles.length;
    ...
    if (fileLength === 0 && addedLength === 1) { // Allow only one document to be selected
      ...
      if (size > 5) {
        ...
        if (isSuccess) {
          this.committeefiles = [selectedFile]; // Replace the array with the single selected file
          this.solutionForm.controls['commitee_statement'].setErrors(null);
          this.Upload_committee_statement()
        } else {
          const statusText = "Please choose a document ('pdf', 'word', 'jpg', 'jpeg', 'png')";
          this._snackBar.open(statusText, 'Close Warning', {
            horizontalPosition: this.horizontalPosition,
  ...
  Upload_statement() {
    const reference = this.Form.value.case_id;
    this.files = this.statementfiles
    if (this.statementfiles.length != 0) {
      this.statementfiles.forEach((elem: any) => {
        ...
        this.generalService.upload(this.statementFormData).subscribe({
          next: (result: any) => {
            ...
            this.grievanceService.create_grievance_statement(data[0]).subscribe({
              next: (result: any) => { },
              error: (err: any) => { this.router.navigate(["/error/internal"]) },
              complete: () => {
                const statusText = " You have successfully uploaded Statement."
                this._snackBar.open(statusText, 'Ok', {
                  horizontalPosition: this.horizontalPosition,
                  verticalPosition: this.verticalPosition,
                });
  ...
  Upload_committee_statement() {
    const reference = this.Form.value.case_id;
    this.files = this.committeefiles
    if (this.committeefiles.length != 0) {
      this.committeefiles.forEach((elem: any) => {
        this.committeeFormData.delete('files')
        const extension = elem.name.split('.').pop().toLowerCase()
        this.committeeFormData.append('files', elem, reference + '.' + extension)
        this.generalService.upload(this.committeeFormData).subscribe({
          next: (result: any) => {
            let data: any[] = []
            data.push({
              statement_name: result[0].hash,
            ...
            this.grievanceService.create_grievance_statement(data[0]).subscribe({
              ...
              complete: () => {
                const statusText = " You have successfully uploaded Investigation Committee Statement."
                this._snackBar.open(statusText, 'Ok', {
                  horizontalPosition: this.horizontalPosition,
                  verticalPosition: this.verticalPosition,
                });
  ...
  create_notification() {
    ...
    this.generalService.create_notification(data[0]).subscribe({
      ...
      complete: () => {
        const statusText = " You have successfully updated the details"
        this._snackBar.open(statusText, 'Ok', {
          horizontalPosition: this.horizontalPosition,
          verticalPosition: this.verticalPosition,
        });
