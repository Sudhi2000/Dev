...
export class ReportComponent implements OnInit {
  ...
  selectedDivision(selectedDivision: any) {
  ...
  create_grievance_draft() {
    if (this.Form.value.anonymous == false) {
      this.grievanceService.create_non_grievance(this.Form.value, this.anonymousForm.value).subscribe({
        next: (result: any) => {
      ...
      this.grievanceService.create_non_grievance_anonymous(this.Form.value, this.selectedDivision).subscribe({
        next: (result: any) => {
          this.Form.controls['id'].setValue(result.data.id)
        },
        error: (err: any) => {
            ...
            this.grievanceService.create_grievance_evidence(data[0]).subscribe({
              ...
              complete: () => {
                ...
                if (this.evidenceCount === this.files.length) {
                  ...
                  Swal.close()
                  this.router.navigate(["/apps/grievance/modify-non-grievance/" + this.Form.value.id])
                }
              }
            })
          },
