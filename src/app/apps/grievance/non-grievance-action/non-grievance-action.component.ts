...
@Component({
  selector: 'app-non-grievance-action',
  templateUrl: './non-grievance-action.component.html',
  styleUrls: ['./non-grievance-action.component.scss'],
  providers: [
    { provide: DateAdapter, useClass: MomentDateAdapter, deps: [MAT_DATE_LOCALE] },
    { provide: MAT_DATE_FORMATS, useValue: MY_FORMATS }
  ]
})
export class NonGrievanceActionComponent implements OnInit {
      ...
      next: (result: any) => {
        ...
        if (!matchFound || matchFound !== true) {
          ...
          if (evidence__data?.length > 0) {
            if (evidence__data[0].evidece_id) {
  ...
  update_completion_status() {
    this.grievanceService.update_non_grievance_completion_status(this.Form.value.id).subscribe({
      next: (result: any) => { },
      error: (err: any) => {
        this.router.navigate(["/error/internal"])
      },
  ...
  submit() {
    this.update_non_grievance_solution()
    // const formStatus = this.solutionForm.valid
    // if (formStatus) {
    //   Swal.fire({
    //     title: 'Are you sure?',
  ...
  update_non_grievance_solution() {

    this.grievanceService.update_non_grievance_solution(this.Form.value.id, this.solutionForm.value).subscribe({
      next: (result: any) => {
      },
      error: (err: any) => {
        this.router.navigate(["/error/internal"])
  ...
  create_notification() {
    ...
    this.generalService.create_notification(data[0]).subscribe({
      ...
      complete: () => {
        ...
        //   imageWidth: 250,
        //   text: "You have successfully Updated the Grievance Solution details.",
        //   showCancelButton: false,
        // })
        // this.router.navigate(["/apps/grievance/assigned-tasks"])
        const statusText = "You have updated the details successfully"
        this._snackBar.open(statusText, 'OK', {
          horizontalPosition: this.horizontalPosition,
          verticalPosition: this.verticalPosition,
        });
