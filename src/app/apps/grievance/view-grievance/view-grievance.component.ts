...
@Component({
  ...
  styleUrls: ['./view-grievance.component.scss'],
  providers: [
    ...
    this.grievanceService.get_grievance_details(reference).subscribe({
      next: (result: any) => {
        ...
        if (!matchFound || matchFound !== true) {
          ...
          this.solutionForm.controls['union_representative'].setValue(result.data[0].attributes.union_representative?.data?.attributes.employee_name)
