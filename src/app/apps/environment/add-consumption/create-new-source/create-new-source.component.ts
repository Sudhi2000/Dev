...
@Component({
  selector: 'app-create-new-source',
  templateUrl: './create-new-source.component.html',
  styleUrls: ['./create-new-source.component.scss']
})
export class CreateNewSourceComponent implements OnInit {
  ...
  ngOnInit() {
    this.Form = this.formBuilder.group({
      ...
      unit: ['', this.data.consumptionCategory !== 'Air Emission' ? [Validators.required] : []],
    });
    this.configuration()
  }
    ...
    this.environmentService.get_consumption_dropdown_values(module).subscribe({
      next: (result: any) => {
        ...
        const units = this.consumptionDropDownValues.filter(function (elem: any) {
          return elem.attributes.Category === 'Unit'&& elem.attributes.filter==='Waste';
        });
