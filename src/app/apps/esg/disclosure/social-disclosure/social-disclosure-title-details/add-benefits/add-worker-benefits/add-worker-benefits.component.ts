...
@Component({
  selector: 'app-add-worker-benefits',
  templateUrl: './add-worker-benefits.component.html',
  styleUrls: ['./add-worker-benefits.component.scss']
})
export class AddWorkerBenefitsComponent implements OnInit {
  ...
  constructor(private router: Router,
    ...
    public dialogRef: MatDialogRef<AddWorkerBenefitsComponent>) { }
  ...
  ngOnInit() {
    ...
    this.Form = this.formBuilder.group({
      ...
      workers: [this.data.data.workers || ''],
  ...
  submit() {
    ...
    if (this.data.mode === 'modify' && this.data.data.id) {
      ...
      formData.append('Worker_benefits', JSON.stringify(this.Form.value))
      // formData.append('form_value', JSON.stringify(this.socialDisclosureForm.value))
      this.esgService.updateSocDisWorkerBenefits(formData).subscribe({
        next: (result: any) => {
        },
        error: (err: any) => {
