...
@Component({
  selector: 'app-add-worker-turnover',
  templateUrl: './add-worker-turnover.component.html',
  styleUrls: ['./add-worker-turnover.component.scss']
})
export class AddWorkerTurnoverComponent implements OnInit {
 ...
 Form: FormGroup
  ...
  constructor(private router: Router,
    ...
    public dialogRef: MatDialogRef<AddWorkerTurnoverComponent>) { }
  ...
  ngOnInit() {
    ...
    this.Form = this.formBuilder.group({
      ...
      worker_turnover: [this.data.data.worker_turnover || ''],
  ...
  submit() {
    if (this.data.mode === 'modify' && this.data.data.id) {
      ...
      formData.append('worker_turnover', JSON.stringify(this.Form.value))
      // formData.append('form_value', JSON.stringify(this.socialDisclosureForm.value))
      this.esgService.updateSocDisTitleWorkerTurnover(formData).subscribe({
        next: (result: any) => {
        },
        error: (err: any) => {
