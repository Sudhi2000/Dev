...
@Component({
  selector: 'app-add-employee-perfomance-review',
  templateUrl: './add-employee-perfomance-review.component.html',
  styleUrls: ['./add-employee-perfomance-review.component.scss']
})
export class AddEmployeePerfomanceReviewComponent implements OnInit {
 ...
 Form: FormGroup
  ...
  constructor(private router: Router,
    ...
    public dialogRef: MatDialogRef<AddEmployeePerfomanceReviewComponent>) { }
  ...
  submit() {
    if (this.data.mode === 'modify' && this.data.data.id) {
      ...
      formData.append('employee_perfomance_review', JSON.stringify(this.Form.value))
      // formData.append('form_value', JSON.stringify(this.socialDisclosureForm.value))
      this.esgService.updateSocDisTitleEmployeePerfomanceReviews(formData).subscribe({
        next: (result: any) => {
        },
        error: (err: any) => {
