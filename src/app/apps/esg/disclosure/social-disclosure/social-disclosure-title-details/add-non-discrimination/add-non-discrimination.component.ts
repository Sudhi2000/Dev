...
@Component({
  selector: 'app-add-non-discrimination',
  templateUrl: './add-non-discrimination.component.html',
  styleUrls: ['./add-non-discrimination.component.scss']
})
export class AddNonDiscriminationComponent implements OnInit {
 ...
 Form: FormGroup
  ...
  constructor(private router: Router,
    ...
    public dialogRef: MatDialogRef<AddNonDiscriminationComponent>) { }
  ...
  ngOnInit() {
    this.Form = this.formBuilder.group({
      ...
      types_of_discrimination: [this.data.data.types_of_discrimination || '', Validators.required],
  ...
  submit() {
    if (this.data.mode === 'modify' && this.data.data.id) {
      ...
      formData.append('non_discrimination', JSON.stringify(this.Form.value))
      // formData.append('form_value', JSON.stringify(this.socialDisclosureForm.value))
      this.esgService.updateSocDisTitleNonDiscrimination(formData).subscribe({
        next: (result: any) => {
        },
        error: (err: any) => {
