...
@Component({
  selector: 'app-add-new-worker-hires',
  templateUrl: './add-new-worker-hires.component.html',
  styleUrls: ['./add-new-worker-hires.component.scss']
})
export class AddNewWorkerHiresComponent implements OnInit {
  Form: FormGroup
  lov: any[] = []
  ...
  constructor(private router: Router,
    ...
    public dialogRef: MatDialogRef<AddNewWorkerHiresComponent>) { }
  ...
  ngOnInit() {
    this.Form = this.formBuilder.group({
      ...
      new_worker_hires: [this.data.data.new_worker_hires || ''],
  ...
  submit() {
    if (this.data.mode === 'modify' && this.data.data.id) {
      ...
      formData.append('new_worker_hire', JSON.stringify(this.Form.value))
      // formData.append('form_value', JSON.stringify(this.socialDisclosureForm.value))
      this.esgService.updateSocDisTitleWorkerHire(formData).subscribe({
        next: (result: any) => {
        },
        error: (err: any) => {
  ...
  new_region() {
    this.dialog.open(CreateNewRegionComponent, { width: "500px", data: { userID: this.data.userID, mode: 'create' } }).afterClosed().subscribe((data: any) => {
      if(data){this.lov[0].value.push(data)
