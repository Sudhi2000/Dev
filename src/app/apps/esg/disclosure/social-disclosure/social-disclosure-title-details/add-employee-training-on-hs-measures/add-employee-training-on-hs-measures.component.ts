...
@Component({
  selector: 'app-add-employee-training-on-hs-measures',
  templateUrl: './add-employee-training-on-hs-measures.component.html',
  styleUrls: ['./add-employee-training-on-hs-measures.component.scss']
})
export class AddEmployeeTrainingOnHsMeasuresComponent implements OnInit {
  Form: FormGroup
  lov: any[] = []
  ...
  constructor(private router: Router,
    ...
    public dialogRef: MatDialogRef<AddEmployeeTrainingOnHsMeasuresComponent>) { }
  ...
  submit() {
    if (this.data.mode === 'modify' && this.data.data.id) {
      ...
      formData.append('employee_hs_training_measure', JSON.stringify(this.Form.value))
      // formData.append('form_value', JSON.stringify(this.socialDisclosureForm.value))
      this.esgService.updateSocDisTitleTrainingOnHealthSafetyMeasures(formData).subscribe({
        next: (result: any) => {
        },
        error: (err: any) => {
