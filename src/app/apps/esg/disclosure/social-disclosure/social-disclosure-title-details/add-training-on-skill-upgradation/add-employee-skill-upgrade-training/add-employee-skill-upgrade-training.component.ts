...
@Component({
  selector: 'app-add-employee-skill-upgrade-training',
  templateUrl: './add-employee-skill-upgrade-training.component.html',
  styleUrls: ['./add-employee-skill-upgrade-training.component.scss']
})
export class AddEmployeeSkillUpgradeTrainingComponent implements OnInit {
  ...
  constructor(private router: Router,
    ...
    public dialogRef: MatDialogRef<AddEmployeeSkillUpgradeTrainingComponent>) { }
  ...
  submit() {
    if (this.data.mode === 'modify' && this.data.data.id) {
      ...
      formData.append('employee_skill_upgrade_training', JSON.stringify(this.Form.value))
      // formData.append('form_value', JSON.stringify(this.socialDisclosureForm.value))
      this.esgService.updateSocDisTitleEmployeeSkillUpgradeTraining(formData).subscribe({
        next: (result: any) => {
        },
        error: (err: any) => {
