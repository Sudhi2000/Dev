...
@Component({
  selector: 'app-add-dif-abled-workers',
  templateUrl: './add-dif-abled-workers.component.html',
  styleUrls: ['./add-dif-abled-workers.component.scss']
})
export class AddDifAbledWorkersComponent implements OnInit {
  ...
  constructor(private router: Router,
    ...
    public dialogRef: MatDialogRef<AddDifAbledWorkersComponent>) { }
  ...
  ngOnInit() {
    ...
    this.Form = this.formBuilder.group({
      ...
      differently_abled_worker: [this.data.data.differently_abled_worker || ''],
  ...
  submit() {
    if (this.data.mode === 'modify' && this.data.data.id) {
      ...
      formData.append('dif_abled_worker', JSON.stringify(this.Form.value))
      // formData.append('form_value', JSON.stringify(this.socialDisclosureForm.value))
      this.esgService.updateSocDisDifAbledWorker(formData).subscribe({
        next: (result: any) => {
        },
        error: (err: any) => {
