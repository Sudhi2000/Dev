...
export class AddComplaintsComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.Form = this.formBuilder.group({
      ...
      stakeholder_group: [this.data.addData.stakeholder_group || '', Validators.required],
      complaints_received: [this.data.addData.complaints_received || null, Validators.required],
      complaints_pending: [this.data.addData.complaints_pending || null, Validators.required],
      remarks: [this.data.addData.remarks || '', Validators.required],
    })
  ...
  submit() {
    ...
    if (this.data.mode === 'modify' && this.data.addData.id) {
      ...
      formData.append('grievances_details', JSON.stringify(this.Form.value))
      this.esgService.updateGovGrievances(formData).subscribe({
        next: (result: any) => {
