...
@Component({
  selector: 'app-chemical-request-history',
  templateUrl: './chemical-request-history.component.html',
  styleUrls: ['./chemical-request-history.component.scss']
})
export class ChemicalRequestHistoryComponent implements OnInit {
        ...
        if (status === false) {
          ...
          if (this.unitSpecific) {
            ...
            if (this.corporateUser) {
              ...
              this.get_chemical_request({ pageIndex: prevPageIndex, pageSize: this.pageSize, length: 0 })
              ...
              const prevPageIndex = Number(localStorage.getItem('pageIndex')) || 0;
              this.get_chemical_unit_specific_request({ pageIndex: prevPageIndex, pageSize: this.pageSize, length: 0 })
            }
          }
          else {
            const prevPageIndex = Number(localStorage.getItem('pageIndex')) || 0;
  ...
  get_chemical_unit_specific_request(pageEvent: PageEvent) {
    this.isLoading = true;
    const startIndex = pageEvent.pageIndex * pageEvent.pageSize;
    const endIndex = startIndex + pageEvent.pageSize;
    localStorage.setItem('pageIndex', JSON.stringify(pageEvent.pageIndex));
    this.chemicalService.get_chemical_unit_specific_request(startIndex, pageEvent.pageSize, this.userDivision).subscribe({
      next: (result: any) => {
  ...
  get_chemical_request(pageEvent: PageEvent) {
    this.isLoading = true;
    const startIndex = pageEvent.pageIndex * pageEvent.pageSize;
    const endIndex = startIndex + pageEvent.pageSize;
    localStorage.setItem('pageIndex', JSON.stringify(pageEvent.pageIndex));
    this.chemicalService.get_chemical_request_pag(startIndex, pageEvent.pageSize).subscribe({
      next: (result: any) => {
  ...
  modify(data: any) {
    if (data.status === "Rejected" || data.status === "Draft") {
      ...
      this.chemicalService.get_chemical_details_refe(data?.reference_number).subscribe({
        next: (result: any) => {
          this.router.navigate(["/apps/chemical-management/modify-request/" + result.data[0].id])
  ...
  generate() {
    ...
    this.isLoading = true;
    if (!this.corporateUser) {
      this.chemicalService.get_chemical_unit_specific_request_search(this.serachReference, this.userDivision).subscribe({
        next: (result: any) => {
          const data = result.data.map((elem: any) => elem.attributes);
          this.chemicalRequest = data
        },
      ...
      this.chemicalService.get_chemical_request_search(this.serachReference).subscribe({
        next: (result: any) => {
          const data = result.data.map((elem: any) => elem.attributes);
          this.chemicalRequest = data
        },
