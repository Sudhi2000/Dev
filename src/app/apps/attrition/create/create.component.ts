...
export class CreateComponent implements OnInit {
    ...
    if (formStatus) {
      Swal.fire({
        ...
        if (result.isConfirmed) {
          ...
          const responsiblePersonValue = this.Form.controls['responsible_person'].value;
  ...
  create_attrition() {
    ...
    this.attritionService.create_attrition(this.Form.value).subscribe({
      next: (result: any) => {
        ...
        if (status === "Draft" && id) {
          this.router.navigate([`/apps/attrition/modify`, id]);
        } else {
      ...
      complete: () => {
        Swal.fire({
          title: 'Attrition Details Created',
          imageUrl: "assets/images/success.gif",
          imageWidth: 250,
          text: "You have successfully added a Attrition details.",
