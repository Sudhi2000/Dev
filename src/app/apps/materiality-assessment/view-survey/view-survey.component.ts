...
@ViewChildren('sortableItem') sortableList: QueryList<ElementRef>;
  ...
  get_details() {
    this.materialityService.get_materiality_survey(this.Form.value.reference_id).subscribe({
      next: (result: any) => {
        ...
        this.Form.controls['reported_date'].setValue(result.data[0].attributes.created_date)
        ...
        // this.selected_topics = result?.data[0]?.attributes
        console.log("Selected topics: ",result.data[0]?.attributes)
  ...
  get_attended_survey(){
    this.materialityService.get_stakeholder_list(this.Form.value.reference_id).subscribe({
      next:(result: any) => {
        this.attendedsurvey = result.data.length
      },
      error:(err: any) => {},
      complete:() => {}
    })
...
let requestData = {
  ...
  addStakeholder() {
    // this.dialog.open(UpdateStakeholderComponent).afterClosed().subscribe(data => {
    //   if (data) {
    ...
    //       next: (result: any) => {
    //         console.log("Stakeholder result: ", result)
  ...
  schedule_survey() {
    this.materialityService.update_survey(this.Form.value).subscribe({
      next: (result: any) => {
        const statusText = "Survey updated sucessfully.";
