...
import { accident_corrective_actions, accident_register, ListColumn } from 'src/app/services/schemas';
...
export class CorrectiveActionsComponent implements OnInit {
  ...
  orgID: string
  userID: number
  subject$: ReplaySubject<accident_corrective_actions[]> = new ReplaySubject<accident_corrective_actions[]>(1);
  data$: Observable<accident_corrective_actions[]> = this.subject$.asObservable();
  customers: accident_corrective_actions[];
  totalItems = 0;
  isLoading = true;
  ...
  pageSize = 10;
  dataSource: MatTableDataSource<accident_corrective_actions>;
  ...
  get_corrective_actions(pageEvent: PageEvent) {
    ...
    this.accidentService.get_assigned_corrective_action(this.userID, startIndex, pageEvent.pageSize).subscribe({
      next: (result: any) => {
  ...
  private prepareView() {
    this.dataSource = new MatTableDataSource<accident_corrective_actions>(this.correctiveActions);
