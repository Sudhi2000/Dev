...
import { CreateUpdateAssignmentComponent } from './create-update-assignment/create-update-assignment.component';
...
export class ActionComponent implements OnInit {
    ...
    this.equipmentService.get_equipment_details(reference).subscribe({
      next: (result: any) => {
        ...
        if (result.data[0].attributes.assignment.data.length > 0) {
          ...
          this.assignmentList = result.data[0].attributes.assignment.data.sort((a: any, b: any) => b.id - a.id);
  ...
  createAssignment() {
    this.dialog.open(CreateUpdateAssignmentComponent).afterClosed().subscribe(data => {
      if (data) {
        this.showProgressPopup();
        this.equipmentService.create_assignment(data, this.Form.value.id).subscribe({
          next: (result: any) => { },
          error: (err: any) => {
            this.router.navigate(["/error/internal"])
          },
          complete: () => {
            const statusText = "Assignment details added"
  ...
  updateAssignment(assignData: any) {
    this.dialog.open(CreateUpdateAssignmentComponent, { data: assignData }).afterClosed().subscribe(data => {
      if (data) {
        //this.showProgressPopup();
        this.equipmentService.update_assignment(data).subscribe({
          next: (result: any) => { },
          error: (err: any) => { },
          complete: () => {
            const statusText = "Assignment details updated"
            this._snackBar.open(statusText, 'OK', {
              horizontalPosition: this.horizontalPosition,
              verticalPosition: this.verticalPosition,
            });
  ...
  deleteAssignment(data: any) {
    const id = data.id
    this.showProgressPopup();
    this.equipmentService.delete_assignment(id).subscribe({
      next: (result: any) => { },
      error: (err: any) => { },
      complete: () => {
        const statusText = "Assignment details removed"
        this._snackBar.open(statusText, 'OK', {
          horizontalPosition: this.horizontalPosition,
          verticalPosition: this.verticalPosition,
        });
