...
export class AssignedTasksComponent implements OnInit {
        ...
        if (status === false) {
          ...
          if (this.unitSpecific) {
            ...
            if (this.corporateUser) {
              ...
              this.get_target_setting_assigned({ pageIndex: prevPageIndex, pageSize: this.pageSize, length: 0 })
              ...
              const prevPageIndex = Number(localStorage.getItem('pageIndex')) || 0;
              this.get_target_setting_unit_specific_assigned({ pageIndex: prevPageIndex, pageSize: this.pageSize, length: 0 })
            }
          }
          else {
            const prevPageIndex = Number(localStorage.getItem('pageIndex')) || 0;
  ...
  get_target_setting_assigned(pageEvent: PageEvent) {
    this.isLoading = true;
    const startIndex = pageEvent.pageIndex * pageEvent.pageSize;
    const endIndex = startIndex + pageEvent.pageSize;
    localStorage.setItem('pageIndex', JSON.stringify(pageEvent.pageIndex));
    this.targetSettingService.get_target_setting_assigned(this.userID, startIndex, pageEvent.pageSize).subscribe({
      next: (result: any) => {
        let data: any[] = []
  ...
  get_target_setting_unit_specific_assigned(pageEvent: PageEvent) {
    this.isLoading = true;
    const startIndex = pageEvent.pageIndex * pageEvent.pageSize;
    const endIndex = startIndex + pageEvent.pageSize;
    localStorage.setItem('pageIndex', JSON.stringify(pageEvent.pageIndex));
    this.targetSettingService.get_target_setting_unit_specific_assigned(this.userID, startIndex, pageEvent.pageSize, this.userDivision).subscribe({
      next: (result: any) => {
        let data: any[] = []
        result.data.forEach((elem: any) => {
          const baseline = elem.attributes.baseline_consumption
  ...
  generate() {
    ...
    if (this.unitSpecific && !this.corporateUser) {
      this.targetSettingService.get_target_setting_unit_specific_assigned_search(this.serachReference, this.userID, this.userDivision).subscribe({
        next: (result: any) => {
          console.log(result)
      ...
      this.targetSettingService.get_target_setting_assigned_search(this.serachReference, this.userID).subscribe({
        next: (result: any) => {
          console.log(result)
  ...
  action(data: any) {
    ...
    if (data.status === 'Open') {
      const reference = data.reference_number
      this.router.navigate(["apps/target-setting/action/" + reference])
    } else if (data.status === 'Approved' || data.status === "In-Progress") {
      const reference = data.reference_number
      this.router.navigate(["apps/target-setting/progress/" + reference])
    }
  }
  ngOnDestroy(): void {
