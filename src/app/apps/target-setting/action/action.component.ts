...
export class ActionComponent implements OnInit {
  ...
  get_target_setting_details() {
    ...
    this.targetService.get_target_setting_details(reference).subscribe({
      next: (result: any) => {
        ...
        if ((result.data[0].attributes.status === "Open" || result.data[0].attributes.status === "In-Progress") && (matchFound || matchFound !== false)
          // || result.data[0].attributes.status === "Approved"
        ) {
          ...
          if (this.Form.value.category) { this.get_possible_category(this.Form.value.category) }
          ...
          this.Form.controls['createdby_id'].setValue(result.data[0].attributes.created_By.data.id)
          ...
          this.Form.controls['expected_saving'].disable()
      ...
      this.dialog.open(NewPossibleCategoryComponent, { width: '700px', data: dialogData }).afterClosed().subscribe(data => {
        if (data) {
          this.targetService.get_possible_category().subscribe({
            next: (result: any) => {
              const filteredResult = result.data.filter((item: any) => {
                return item.attributes.category === this.Form.value.category;
              });
              this.possibilityCategory = filteredResult.sort((a: any, b: any) => a.attributes.possible_category.localeCompare(b.attributes.possible_category));

              if (filteredResult.length > 0) {
                this.Form.controls['possibility_category'].setValue(filteredResult[0].attributes.possible_category);
                this.Form.controls['categoryId'].setValue(filteredResult[0].id);
              }
            },
            error: (err: any) => { },
            complete: () => {
  ...
  category() {
    ...
    this.sources = source
    if (this.Form.value.possibility_category) {
      this.Form.controls['possibility_category'].reset()
    }
  ...
  submit() {
    ...
    this.targetService.update_target_setting(this.Form.value).subscribe({
      next: (result: any) => { },
      error: (err: any) => { },
      complete: () => {
        Swal.close()
        const statusText = "Target details updated"
        this._snackBar.open(statusText, 'Ok', {
          horizontalPosition: this.horizontalPosition,
          verticalPosition: this.verticalPosition,
        });
    ...
    this.targetService.update_target(this.Form.value).subscribe({
      next: (result: any) => {
        if (result.data.attributes.status === "Approved") {
          this.create_responsible_notification(result.data.attributes.status)
        } else if (result.data.attributes.status === "Rejected") {
          this.create_creator_notification(result.data.attributes.status)
        } else if (result.data.attributes.status === "Hold") {
          this.create_creator_notification(result.data.attributes.status)
        }
      },
      error: (err: any) => {
  ...
  create_responsible_notification(status: any) {
    let data: any[] = []
    data.push({
      module: "Target Setting",
      action: 'Assigned for resolution:',
      ...
      profile: this.Form.value.approver
    })
    this.generalService.create_notification(data[0]).subscribe({
      ...
      complete: () => {
        this.create_creator_notification(status)
      }
    })
  }

  create_creator_notification(status: any) {
    let data: any[] = []
    data.push({
      module: "Target Setting",
      action: 'Target setting ' + status + ' :',
      reference_number: this.Form.value.reference_number,
      userID: this.Form.value.createdby_id,
      access_link: "/apps/target-setting/view/",
      profile: this.Form.value.approver
    })
    this.generalService.create_notification(data[0]).subscribe({
      next: (result: any) => { },
